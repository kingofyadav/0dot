...
2. **No page should take more than three taps/clicks to reach** from anywhere in the primary nav. Today's nav is small enough this is trivially true; re-check this explicitly as `docs/foundations/NAVIGATION.md`'s sidebar/bottom-nav structure grows.
...
8. **Errors are specific and actionable**, never generic. Current examples to hold the line on: `"Username must be 3-30 characters: letters, numbers, underscore only."` not `"Invalid input."` Every new validation error follows this specificity bar.
9. **Destructive actions require explicit confirmation**, and the confirmation UI explains the consequence, not just "are you sure?" (Not yet exercised anywhere in the current codebase — no delete-account or delete-community flow exists yet. This is the rule to build those against when they land in Phase 12.)
