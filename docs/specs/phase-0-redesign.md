...
| **5. Mobile** | `redesign/phase-5-mobile` | Expo app parity (§6). **In progress:** `mobile/src/theme.ts` gained `colors.surface2` + `colors.borderStrong` + `space.{10,12,16}` mirroring the web redesign tokens; `Card` and the shared inputs (`SearchBar`, `PasswordInput`, edit-profile fields) adopted `borderStrong` + a real 1px edge; `EmptyState` brought to parity with the web component (icon in a soft accent disc, `title` + `description` + `action`, `message`→`title` back-compat, `onRetry` kept). **Then (2026-08-29):** profile-header cover scrim landed — `expo-linear-gradient` added, a bottom-weighted `0.72 → 0.36 → transparent` overlay mirroring the web `--overlay-scrim` / `.profileCover::after` stops, `pointerEvents="none"` so the cover lightbox tap survives; `PostRow` + the post-detail stats row brought to web `.postActionsRow` parity — liked state is now `danger` (red filled heart) not `accent` to match web's `.postAction[data-like]`, a hairline top divider above the action zone, zero counts hidden (was rendering "0 0 0"), and a shared `formatCount` util (`999+` cap, mirrors web `src/lib/format.ts`). Mobile `tsc` + `jest` (58 tests) + root lint green. **Still open:** a full screen-by-screen visual pass on device. |
