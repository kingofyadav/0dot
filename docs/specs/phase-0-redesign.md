...
| **5. Mobile** | `redesign/phase-5-mobile` | Expo app parity (§6). **In progress:** `mobile/src/theme.ts` gained `colors.surface2` + `colors.borderStrong` + `space.{10,12,16}` mirroring the web redesign tokens; `Card` and the shared inputs (`SearchBar`, `PasswordInput`, edit-profile fields) adopted `borderStrong` + a real 1px edge; `EmptyState` brought to parity with the web component (icon in a soft accent disc, `title` + `description` + `action`, `message`→`title` back-compat, `onRetry` kept). Mobile `tsc` + `jest` (58 tests) + root lint green. **Still open:** a cover scrim on the profile header (needs `expo-linear-gradient`), the `PostRow` action treatment (already close — inline stat buttons with press-springs), and a full screen-by-screen visual pass on device. |
