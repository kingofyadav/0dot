...
| **1. Components** | `redesign/phase-1-components` | §5, all eight items, each with a styleguide entry and skeleton. **In progress:** EmptyState (icon/title/description), PageHeader, `Icon`, `.card`/`.section`, and the PostCard action row (`.postAction`) landed with styleguide entries. Remaining: post-byline avatars + Report/Quote overflow menu, the full input set (Select / file-upload / Switch / Checkbox / Radio / auto-grow Textarea), Buttons reconciliation, Tabs/Popover/Tooltip motion alignment, shape-matched skeletons. |
| **2. App surfaces** | `redesign/phase-2-surfaces` | Feed, Profile, Explore, Settings, rail, nav (§6). **In progress:** nav (colour not opacity, active accent bar tying to the header brand bar, 1.75 icon stroke, `.eyebrow` section labels); contextual rail (upgrade card pulled back from loudest-element to a quiet hairline card — D10; `UserListItem` `compact` mode fixing the 320px name truncation); PostCard (`--border-strong` + resting shadow so it reads as a card — D3; trailing Report/owner controls recede until card hover/focus — D4); profile header (`--overlay-scrim` on the cover, `--text-3xl` name, metadata spacing). Remaining: explore structure (topics / people / categories, not a raw post list), feed composer expand-on-focus, settings form-density pass, profile-header structural rework off the fragile overlap math, `--measure-*` container adoption. |
| **3. Storytelling** | `redesign/phase-3-story` | Landing rebuild, auth, marketing layer (§6). **In progress:** the `/` landing gained a below-the-fold story — `MarketingStory` (5 scroll-revealed sections: links / feed / communities & business / developers / closing CTA, each with a small CSS-built visual, `motion-reveal` via `useReveal`) + `MarketingFooter` (real routes only, per NAVIGATION.md rule 2). Hero headline moved to the `--text-4xl`→`--text-5xl` display treatment; `MarketingNav` got a translucent ground. The DigitalHomeVisual identity node stays the one bold element (§4.6). Remaining: `/login` + `/signup` premium polish, and the `/about` / `/help` / pricing / changelog marketing pages (all still "Future" routes per ROADMAP.md — build the pages before linking them). |
