...
3. **The header's tricolor gradient underline (`.siteHeader::after`) is a persistent brand element** — any sidebar/bottom-nav addition should feel like it belongs to the same visual system (see `DESIGN_SYSTEM.md`), not a bolted-on different component library. Redesign Phase 2 extended this: the active sidebar link (`.navLinkActive`) carries a 3px `--accent` left bar so the "you are here" marker reads as the same brand element, not a generic highlight. Inactive links are `--muted-foreground` (colour, not the old `opacity: 0.75` that also dimmed the icons); section labels use the `.eyebrow` treatment.
