...
Done (2026-08): `.authCard`/`.profileCard` are fluid, and the app shell gained a 768px tier (see Status above). The next concrete responsive-work target is the settings shell (`src/app/s/[username]/layout.tsx` + the ~50 pages under it) — it still only branches at 1024px like the rest of the signed-in app, and `layout.tsx`'s own header row still uses inline `style={{...}}` rather than a class (see `DESIGN_SYSTEM.md`'s "known inconsistency to fix"). After that, the remaining ~130 routes (messages, `/c`, `/b`, courses, marketplace, admin) are unstarted — extend outward from `MarketingNav`'s pattern page by page as they're touched, not in one sweep.
