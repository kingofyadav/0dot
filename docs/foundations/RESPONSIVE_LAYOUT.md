...
Status: Foundational document (Priority 6). **Current state: two real implementations landed, most of the app still doesn't use the full scale.** The marketing landing sections (`src/components/marketing/*`, `"/"` only — `MarketingNav`) are the first component in the codebase to implement the breakpoint scale below, mobile-first, at every listed width. `.authCard`/`.profileCard` (2026-08) are fluid (`width: 100%` with their `max-width` — 380px/640px — kept as a ceiling, not a fixed box) rather than fixed-width boxes; the app shell (`SiteHeader`/`Sidebar`/`MobileBottomNav`) also gained a real `--bp-md` (768px) tier (inline search in the mobile header) on top of its existing `--bp-lg` (1024px) desktop-shell cutover. Everywhere else (most of the ~140 signed-in-app routes) still only branches once, at 1024px. This document defines the target scale so new UI work builds it in rather than retrofitting later; `MarketingNav` is the fullest reference example to follow.
...
Done (2026-08): `.authCard`/`.profileCard` are fluid; the app shell has a 768px tier; the settings shell (`src/app/s/[username]/layout.tsx` + the ~40 pages under it) was moved off inline `style={{...}}` onto `.settingsShell`/`.settingsHeaderRow`/`.settingsContent` classes (layout.tsx has zero inline styles now), and `.stack`/`.row` utility classes exist (see `DESIGN_CONSISTENCY.md`). The settings shell still only branches at 1024px for its two-column nav+content split — genuine multi-breakpoint responsive behavior there is still open. The remaining ~130 routes (messages, `/c`, `/b`, courses, marketplace, admin) are largely unstarted — extend outward from `MarketingNav`'s pattern page by page as they're touched, not in one sweep.
