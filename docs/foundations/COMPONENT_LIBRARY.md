...
| Marketing nav | `src/components/marketing/MarketingNav.tsx` | Landing-page-only (`"/"`) header: logo, Log in / Create your 0dot, `<details>`-based mobile menu. Not `SiteHeader` — this sits above the hero on the one page that doesn't already have its own auth form front and center. **Redesign Phase 3** gave it a translucent blurred ground so it stays legible over the story below. |
| Marketing story | `src/components/marketing/MarketingStory.tsx` | **Redesign Phase 3.** `"/"` below-the-fold: five scroll-revealed sections (links / feed / communities & business / developers / closing CTA), each an `.eyebrow` + `.display-3` + lead + one small CSS-built visual. `motion-reveal` via `useReveal`. |
| Marketing footer | `src/components/marketing/MarketingFooter.tsx` | **Redesign Phase 3.** Footer for the logged-out marketing pages (`/`, `/about`) — brand mark + four link groups, real routes only (`/help`, `/blog`, pricing are still Future per ROADMAP.md). |
